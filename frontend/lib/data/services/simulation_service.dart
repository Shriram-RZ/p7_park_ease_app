...
class SimulationService {
  ...
  void _tick() {
    ...
    for (final s in toMutate) {
      ...
      // Local-only nudges for visual liveliness — never written to the server
      // (real reservations go through ParkingRepository.reserve/free).
      switch (s.status) {
        case SlotStatus.available:
          if (r < pressure) _parking.updateStatus(s.id, SlotStatus.occupied);
          break;
        case SlotStatus.occupied:
          if (r < pressure * 0.7) {
            _parking.updateStatus(s.id, SlotStatus.available);
          }
          break;
        case SlotStatus.reserved:
          if (r < 0.15) _parking.updateStatus(s.id, SlotStatus.occupied);
          break;
